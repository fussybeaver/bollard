...
pub mod buildkit_test {
        ...
        pub async fn cleanup(&mut self) -> Result<(), Error> {
            ...
            let result = driver
                ...
                .map_err(|error| Error::IOError {
                    err: std::io::Error::other(format!("BuildKit fixture cleanup failed: {error}")),
                });
            if result.is_ok() {
                self.driver = None;
                self.cleaned = true;
            ...
            let thread = std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    match runtime {
                        Ok(runtime) => {
                            if let Err(error) =
                                runtime.block_on(driver.remove(DockerContainerRemoveOptions::new()))
                            {
                                eprintln!("BuildKit fixture cleanup failed for {cleanup_name}: {error}");
                            }
                        }
                        Err(error) => eprintln!(
    ...
    pub async fn record_version(
        ...
        if resolved_image_id.is_empty() {
            return Err(validation_error("BuildKit image has no resolved image ID"));
        }
        if buildkitd_version.is_empty() || buildctl_version.is_empty() {
            return Err(validation_error(
                "BuildKit version probe returned no output",
            ));
        }
