...
fn golden(name: &str) -> pb::Definition {
    let bytes: &[u8] = match name {
        ...
        "ssh_explicit_ordering" => {
            include_bytes!("../testdata/golden/ssh_explicit_ordering.llb.pb")
        }
        "exec_network_host" => {
            include_bytes!("../testdata/golden/exec_network_host.llb.pb")
        }
        "exec_security_insecure" => {
            include_bytes!("../testdata/golden/exec_security_insecure.llb.pb")
        }
...
#[test]
fn mutation_ssh_option_data_is_detected() {
    let left = golden("ssh_explicit_ordering");
    let mut right = left.clone();
    common::parity::remap_after_mutation(&mut right, |op, index| {
        if index == 1 {
            if let Some(pb::op::Op::Exec(exec)) = op.op.as_mut() {
                exec.mounts[3].ssh_opt.as_mut().unwrap().mode = 0o644;
            }
        }
    });
    ...
    assert!(compare_and_find(
        ...
        &right,
        "semantic",
        "mounts[3].ssh_opt.mode",
    ));
}

#[test]
fn mutation_network_mode_is_detected() {
    let left = golden("exec_network_host");
    let mut right = left.clone();
    common::parity::remap_after_mutation(&mut right, |op, index| {
        if index == 1 {
            if let Some(pb::op::Op::Exec(exec)) = op.op.as_mut() {
                exec.network = pb::NetMode::None as i32;
            }
        }
    });
...
#[test]
fn mutation_security_mode_is_detected() {
    let left = golden("exec_security_insecure");
    let mut right = left.clone();
    common::parity::remap_after_mutation(&mut right, |op, index| {
        if index == 1 {
            if let Some(pb::op::Op::Exec(exec)) = op.op.as_mut() {
                exec.security = pb::SecurityMode::Sandbox as i32;
            }
        }
    });
