...
pub struct Definition {
    ...
    pub(crate) root: Option<Digest>,
}

impl Definition {
    /// Return serialized protobuf operation vertices in topological order.
    pub fn operations(&self) -> &[Vec<u8>] {
        &self.def
    }

    /// Return per-vertex metadata keyed by operation digest.
    pub fn metadata(&self) -> &BTreeMap<String, pb::OpMetadata> {
        &self.metadata
    }

    /// Return source mapping information, if present.
    pub fn source(&self) -> Option<&pb::Source> {
        self.source.as_ref()
    }

    /// Return the digest of the real graph head.
