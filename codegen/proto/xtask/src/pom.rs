...
pub fn parse_input_spec(contents: &str) -> Result<MobyInputSpec> {
    let document = Document::parse(contents)
        .map_err(|error| anyhow!("could not parse pom.xml: {error}"))?;
    ...
    let url = match input_specs.as_slice() {
        [] => return Err(anyhow!("pom.xml does not contain an inputSpec element")),
        [url] if url.is_empty() => return Err(anyhow!("pom.xml inputSpec is empty")),
        [url] => url.clone(),
        _ => return Err(anyhow!("pom.xml contains multiple inputSpec elements")),
    };

    let parsed = Url::parse(&url)
        .map_err(|error| anyhow!("invalid pom.xml inputSpec URL: {error}"))?;
    ...
    let segments: Vec<&str> = parsed
        .path_segments()
        .ok_or_else(|| anyhow!("pom.xml inputSpec URL has no path"))?
        .collect();
    ...
    let api_index = segments
        ...
        .ok_or_else(|| anyhow!("pom.xml inputSpec URL must point to api/docs"))?;
    if api_index <= 2 || api_index + 2 >= segments.len() {
        return Err(anyhow!("pom.xml inputSpec URL has an invalid Moby ref"));
    }
