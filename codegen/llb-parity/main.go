...
func main() {
	...
	cases := []fixtureCase{
		...
		{"ssh_default", sshDefault, defaultPlatform(), "linux/amd64"},
		{"ssh_explicit_ordering", sshExplicitOrdering, defaultPlatform(), "linux/amd64"},
		{"exec_network_host", execNetworkHost, defaultPlatform(), "linux/amd64"},
		{"exec_security_insecure", execSecurityInsecure, defaultPlatform(), "linux/amd64"},
	}
...
func sshDefault() llb.State {
	return llb.Image("alpine:latest").
		Run(llb.Shlex("test -S /run/buildkit/ssh_agent.0"), llb.AddSSHSocket()).
		Root()
}

func sshExplicitOrdering() llb.State {
	return llb.Image("alpine:latest").
		With(llb.AddEnv("SSH_AUTH_SOCK", "/caller/agent.sock")).
		Run(
			llb.Shlex("true"),
			llb.AddMount("/z", llb.Scratch(), llb.AsPersistentCacheDir("cache-z", llb.CacheMountShared)),
			llb.AddMount("/a", llb.Scratch(), llb.AsPersistentCacheDir("cache-a", llb.CacheMountShared)),
			llb.AddSSHSocket(llb.SSHID("deploy"), llb.SSHSocketOpt("/run/deploy.sock", 1000, 1001, 0o640), llb.SSHOptional),
			llb.AddSSHSocket(llb.SSHID("build"), llb.SSHSocketOpt("/run/build.sock", 2000, 2001, 0o600)),
		).
		Root()
}

func execNetworkHost() llb.State {
	return llb.Image("alpine:latest").
...
func execSecurityInsecure() llb.State {
	return llb.Image("alpine:latest").
