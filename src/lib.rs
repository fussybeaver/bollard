...
//!
//! Enabled by default:
//! - `docker` - Prefer Docker system socket (`/var/run/docker.sock`) in [`Docker::connect_with_local_defaults`]
...
//! | Feature | Description |
//! |---------|-------------|
//! | `docker` | Prefer Docker system socket in [`Docker::connect_with_local_defaults`] (default) |
//! | `podman` | Prefer Podman sockets (rootless then system) in [`Docker::connect_with_local_defaults`] |
//!
//! When both `docker` and `podman` are enabled, `podman` takes precedence.
//!
//! ### Transport Features
//!
//! | Feature | Description |
...
//! ### Local (recommended)
//!
//! Auto-detect the best available socket. With the `podman` feature enabled,
//! rootless and system Podman sockets are probed before falling back to Docker.
//!
//! ```rust
//! use bollard::Docker;
