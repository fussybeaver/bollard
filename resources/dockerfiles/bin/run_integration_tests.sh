...
docker swarm init
docker run -e RUST_LOG=bollard=trace -e REGISTRY_PASSWORD -e REGISTRY_HTTP_ADDR=localhost:5000 -e BOLLARD_BUILDKIT_TEST_IMAGE -e RUST_TEST_NOCAPTURE -v /var/run/docker.sock:/var/run/docker.sock $DOCKER_PARAMETERS -ti --rm bollard cargo test $@ -- --test-threads 1
